...
def TopK_A_star(self, start_junction_id: str, target_junction_id: str, c: float, plot: bool) -> List[Route]:
    """
    ...
    destination_pos: array = array(self.junctions[target_junction_id].get_position())
    limit: float = c * sum([self.edges[edge_id].attributes["length"] for edge_id in shortest_route.edge_list])
    assert (limit > 0)
    ...
    while not queue.empty():
        ...
        if priority > limit:  # Priority is current length + euclidean distance to target
            ...
            assert (length <= limit)
            other_routes.append(self.route_manager.construct_route(path))
            continue
        for route_id in self.junctions[junction_id].travel(in_route_id):
            distance, neigh_junction_id = self.route_manager.traverse(route_id)
            current_distance: float = length + distance
            if neigh_junction_id not in path:  # Avoid loops
                pos: array = array(self.junctions[neigh_junction_id].get_position())  # Current position

                queue.put((
                    (current_distance + norm(destination_pos - pos)),
                    current_distance, route_id, path + [neigh_junction_id]
                ))
...
def A_star(self, start_junction_id: str, target_junction_id: str) -> Tuple[PriorityQueue, Route]:
    ...
    # -------------------------- Init --------------------------
    destination_pos: array = array(self.junctions[target_junction_id].get_position())
    # priority, distance, in_route_id, path (list of visited junctions)
    queue: PriorityQueue[Tuple[float, float, int, List[str]]] = PriorityQueue()
    queue.put((0, 0, 0, [start_junction_id]))
    # For node n, gScore[n] is the cost of the cheapest path from start to n currently known
    g_score: Dict[str, float] = {junction_id: float("inf") for junction_id in self.junctions}
    ...
    while not queue.empty():
        ...
        if junction_id == target_junction_id:  # Found shortest path
            shortest_route = self.route_manager.construct_route(path)
            break
        for route_id in self.junctions[junction_id].travel(in_route_id):
            distance, neigh_junction_id = self.route_manager.traverse(route_id)
            ...
            if tentative_g_score < g_score[neigh_junction_id]:
                ...
                queue.put((
                    ...
                    tentative_g_score, route_id, path + [neigh_junction_id]
                ))
...
def dijkstra(self, start_junction_id: str, target_junction_id: str = "") -> Tuple[Dict[str, float], Dict[str, str]]:
    ...
    queue: PriorityQueue[Tuple[float, str, int]] = PriorityQueue()
    # {junction_id : distance(float), ...}
    dist: Dict[str, float] = {key: float("inf") for key in self.junctions.keys()}
    # {junction_id : previous_junction_id, ...}
    prev: Dict[str, str] = {key: None for key in dist.keys()}
    # -------------------------- Algorithm --------------------------
    dist[start_junction_id] = 0
    queue.put((0, start_junction_id, 0))
    while not queue.empty():
        priority, junction_id, in_route_id = queue.get()
        ...
        if priority == dist[junction_id]:
            for route_id in self.junctions[junction_id].travel(in_route_id):
                distance, to_junction_id = self.route_manager.traverse(route_id)
                ...
                if current_distance < dist[to_junction_id]:  # Found shorter path, record and add to queue
                    ...
                    queue.put((current_distance, to_junction_id, route_id))
    # -------------------------- Finished --------------------------
    return dist, prev
...
def reconstruct_path(self, target_junction_id: str, dist: Dict[str, float], prev: Dict[str, str]) -> Route:
    ...
    """
    if (target_junction_id in self.junctions) and (dist[target_junction_id] != float("inf")):
        path: List[str] = []
        while target_junction_id:
            path.insert(0, target_junction_id)
            target_junction_id = prev[target_junction_id]
        return self.route_manager.construct_route(path)
    return None
