...
from utc.src.utils.constants import file_exists, get_file_extension, PATH, get_planner, get_file_name
...
class UtcLauncher(PddlLauncher):
    ...
    def generate_scenario(self, scenario: str, network: str = "default", *args, **kwargs) -> None:
        # Checks
        network = self.get_network(scenario, network)
        file_names: List[str] = listdir(PATH.TRACI_SCENARIOS.format(scenario) + "/results")
        if not scenario_is_valid(scenario):
            return
        elif not len(file_names):
            print(f"No results files found in: {file_names}!")
            return
        elif not network:
            return
        # Load network
        if self.graph is None or self.graph.skeleton.map_name != network:
            self.graph = Graph(Skeleton())
            self.graph.loader.load_map(network)
            self.graph.simplify.simplify_graph()
        self.pddl_result = UtcResult()
        ...
        for file in file_names:
            ...
            if not file.endswith(".pddl"):
                print(f"Invalid extension found in file: {file}, must be of type '.pddl'!")
                continue
            extension: str = "".join(get_file_extension(file))
            if extension not in scenarios:
                scenarios[extension] = []
            scenarios[extension].append(get_file_name(file))
        ...
        for extension, files in scenarios:
            pass
